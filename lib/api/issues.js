...
Issues.compare = async (context) => {
  const { issueId, revisionIdCompareOne, revisionIdCompareTwo } = context.params;
  const issue = await Issue.findOne({ 
    where: { id: issueId },
    include: Revision 
  });
  ...
  const revisionOne = issue.revisions.find(revision => revision.id === parseInt(revisionIdCompareOne));
  const revisionTwo = issue.revisions.find(revision => revision.id === parseInt(revisionIdCompareTwo));
  
  let changes = {};
  
  if (parseInt(revisionIdCompareOne) < parseInt(revisionIdCompareTwo)) {
    changes = Object.merge(changes, revisionOne.changes, revisionTwo.changes);
  } else {
    changes = Object.merge(changes, revisionTwo.changes, revisionOne.changes);
  }
